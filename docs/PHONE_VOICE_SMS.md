...
| Décroché silencieux | RTP / bruit de ligne envoyé au VAD avant le bonjour | Le pont coupe le micro jusqu’à `response.done` (ou 4 s) et n’envoie que le slin au-dessus du RMS. Journal : `voice-s2s: session` puis `audio N`. |
