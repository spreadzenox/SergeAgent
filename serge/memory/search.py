...
def memory_search(
    ...
    *,
    ...
    """Recherche libre (loguée, filtrée, redactée).
    ...
    Args:
        ...
        venture: Scope associé au filtre demandé ('' = tous).
        since: ISO min ('' = tout).
        tags: Tags requis (tous présents).
        top_k: Résultats max.

    Returns:
        Dict results [{type,id,score,extrait}] + logged.
    """
    ...
    for doc_type, ref, doc_venture, doc_tags, body, ts, rank in rows:
        if doc_type not in wanted:
            continue
        if venture and doc_venture and doc_venture != venture:
            continue
    ...
    return {'results': results, 'logged': True}
